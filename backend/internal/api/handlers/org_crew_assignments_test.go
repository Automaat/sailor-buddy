...
func TestOrgCrewAssignment_AssignTripCrew(t *testing.T) {
	...
	t.Run("crew member from another scope rejected", func(t *testing.T) {
		m := withOrgRole(&mockQuerier{
...
func TestOrgCrewAssignment_RemoveTripCrew(t *testing.T) {
	t.Run("non-admin forbidden", func(t *testing.T) {
		m := withOrgRole(&mockQuerier{}, "crew")
		resp := orgCrewTestAPI(t, m).DeleteCtx(userCtx(context.Background()), "/orgs/club/trips/3/crew/9")
		if resp.Code != http.StatusForbidden {
			t.Fatalf("got %d, want 403", resp.Code)
		}
	})

	// the path tripID must reach the delete query so an assignment cannot be
	// removed through a trip it does not belong to
	t.Run("admin success scopes by trip", func(t *testing.T) {
		var gotTripID int64
		m := withOrgRole(&mockQuerier{
			deleteOrgTripCrewFn: func(_ context.Context, arg sqlcdb.DeleteOrgTripCrewAssignmentParams) error {
				gotTripID = arg.TripID.Int64
				return nil
			},
		}, "admin")
		resp := orgCrewTestAPI(t, m).DeleteCtx(userCtx(context.Background()), "/orgs/club/trips/3/crew/9")
		...
		if gotTripID != 3 {
			t.Fatalf("delete not scoped by path tripID: got %d, want 3", gotTripID)
		}
	})
}
